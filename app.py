...
重要：为防止 PyInstaller 打包后的 Fork Bomb，所有重型导入（flet, core）
都延迟到 if __name__ == "__main__" 块内执行。
"""
...
def create_app(ft, TaskManager, TaskStatus, WhisperModelManager):
    """
    工厂函数：创建应用类和入口函数

    通过参数注入依赖，避免全局导入触发 Fork Bomb。
    """
...
if __name__ == "__main__":
    ...
    # 2. 修复打包环境下 exit() 未定义的问题
    import builtins
    if not hasattr(builtins, 'exit'):
        builtins.exit = sys.exit

    # 3. 延迟导入 Flet 和 core 模块（避免子进程触发 Fork Bomb）
    import flet as ft
    from core import TaskManager, TaskStatus, WhisperModelManager

    # 4. 创建并启动应用
    main = create_app(ft, TaskManager, TaskStatus, WhisperModelManager)
    ft.app(target=main)
